...
namespace CAT.Model
{
    ...
    class OCandle_v2 : Strategy
    {
        ...
        public override void StartOrReset()
        {
            ...
            _clse = eodTime.Date.AddMinutes(570).Add(
            ...
            _open = _clse.AddMinutes(-setup.TimeFrame);

            if (_clse.AddMinutes(30) > eodTime) eodTime = _clse.AddMinutes(30);
            ...
            _bvalue = _target < 1 ? .16m : .31m;
            _uvalue = _target < 1 ? .71m : .91m;
        ...
        public override Setup Gene2Setup(Setup setup, double[] genes)
        {
            ...
            mySetup.TimeFrame = Math.Min(30, Math.Ceiling(3 + genes[0] * 30));
            mySetup.DynamicLoss = Math.Round(.05 + genes[1], 4);
            mySetup.StaticGain = Math.Round(.1 + genes[2] * .45, 4);
            mySetup.StaticLoss = genes.Length > 3 ? Math.Round(0.05 + genes[3] / 3, 4) : .15;

            if (mySetup.StaticGain >= .5)
            {
                mySetup.StaticGain = .5;
                genes[2] = 1;
            }
        ...
        public override double[] Setup2Gene(Setup setup)
        {
            ...
            myGenes[0] = (setup.TimeFrame - 3) / 27;
            myGenes[1] = setup.DynamicLoss.HasValue ? setup.DynamicLoss.Value - .05 : 1;
            myGenes[2] = setup.StaticGain.HasValue ? setup.StaticGain.Value / .5 - .15 : 1;
